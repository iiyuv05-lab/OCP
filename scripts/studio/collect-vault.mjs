...
export async function collect(root, label = 'Vault', maxTextBytes = 80000) {
  ...
  const idFor = (name) =>
    `source:${label}:${createHash('sha256').update(name).digest('hex').slice(0, 20)}`;
  async function visit(rel, parent) {
    const full = path.join(root, rel),
      stat = await lstat(full);
    if (stat.isSymbolicLink()) {
      omitted.push({ path: rel, reason: 'symlink-not-followed' });
      return;
    }
    ...
    const n = {
      ...
      title: rel ? path.basename(rel) : label,
      tiers: [],
      representations: [],
      source: {
    ...
    if (!isDir) {
      ...
      n.source.sha256 = createHash('sha256').update(bytes).digest('hex');
      const ext = path.extname(rel).toLowerCase();
      n.representations = ['.md', '.txt', '.csv'].includes(ext)
        ? ['CUI']
        : ['.png', '.jpg', '.jpeg', '.webp', '.gif', '.svg'].includes(ext)
          ? ['GUI']
          : ['.tsx', '.jsx', '.css', '.html'].includes(ext)
            ? ['CLI', 'GUI']
            : ['CLI'];
      const textExt = [
        '.md',
      ...
      if (
        ...
        bytes.length <= maxTextBytes &&
        !bytes.includes(0)
      ) {
        n.body = bytes.toString('utf8');
        n.source.bodyStatus = 'loaded';
    ...
    if (parent)
      edges.push({
        ...
        to: id,
        predicate: 'CONTAINS',
        source: { basis: 'filesystem-parent' },
      });
    if (isDir)
      for (const item of (await readdir(full)).sort()) {
        if (
          skip.has(item) ||
          /(^\.env|\.pem$|\.key$|credentials|secret|token\.json)/i.test(item)
        ) {
          omitted.push({
            path: path.join(rel, item),
            reason: 'private-or-generated-exclusion',
          });
          continue;
        }
    ...
    coverage: [
      {
        ...
        fullRemoteVaultScan: false,
        semanticClassification: 'unreviewed',
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  if (!root || !output)
    throw Error('Usage: node collect-vault.mjs ROOT OUTPUT_JSON [LABEL]');
  const graph = await collect(root, label || path.basename(root));
  await writeFile(output, JSON.stringify(graph, null, 2));
  console.log(
    JSON.stringify({
      nodes: graph.nodes.length,
      files: graph.coverage[0].files,
      scope: graph.coverage[0].scope,
    }),
  );
}
