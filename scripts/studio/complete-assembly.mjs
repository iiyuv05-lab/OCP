...
async function edit(name, transform) {
  const old = await readFile(name, 'utf8');
  const next = transform(old);
  if (next !== old) await writeFile(name, next);
}
await edit('public/ocp-studio/core.mjs', (text) => {
  if (text.includes('assembly: assemble(g, work.id)')) return text;
  if (
    !text.includes('    const spec = {') ||
    !text.includes('const sections =')
  )
    throw Error('Reviewed design source drifted');
  return (
    "import { assemble, renderAssembly } from './assembly.mjs';\n" +
    text
      .replace("VERSION = '8.0.1'", "VERSION = '8.0.2'")
      .replace(
        '    const spec = {',
        '    const spec = {\n      assembly: assemble(g, work.id),',
      )
      .replace(
        '</section></main><footer>OCP',
        "</section>${sections ? '' : renderAssembly(d.assembly || [], escapeHTML)}</main><footer>OCP",
      )
      .replace(
        '</style></head><body><header>',
        '.assembly-grid{display:grid;gap:20px}.assembly-part{border:1px solid #ccd5c6;border-radius:10px;padding:20px;background:#fff}.assembly-children{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px;margin-top:18px}.assembly-kind{font:10px monospace;letter-spacing:.14em;color:#526755}.assembly-part p{font-size:14px}.assembly-part h2,.assembly-part h3,.assembly-part h4{font-size:20px;margin:12px 0}</style></head><body><header>',
      )
  );
});
await edit('public/ocp-studio/app.mjs', (text) => {
  if (!text.includes('  designToHTML,'))
    text = text.replace('  digest,', '  digest,\n  designToHTML,');
  text = text.replace(
    "options.images?.[src] || '/' + src",
    "options.images?.[src] || (src.startsWith('/') ? src : '/' + src)",
  );
  text = text.replace(
    "${n.kind === 'code' ? '<iframe",
    "${['code', 'design'].includes(n.kind) ? '<iframe",
  );
  if (
    !text.includes("if (n.kind === 'design') $('#detail-slot iframe').srcdoc")
  )
    text = text.replace(
...
await edit('public/ocp-studio/renderer.mjs', (text) =>
  text.replace(
...
await edit('scripts/studio/install-plmag.mjs', (text) => {
  if (!text.includes("'assembly.mjs'"))
    text = text.replace("  'core.mjs',", "  'core.mjs',\n  'assembly.mjs',");
