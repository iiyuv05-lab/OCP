...
const original = "    const body = nonempty(p.body, '원문'),\n      title = nonempty(p.title || body.slice(0, 90), '제목', 180);";
if (!core.includes('Preserve raw whitespace')) {
  if (!core.includes(original)) throw new Error('Raw capture anchor drifted; review required.');
  core = core.replace(original, "    // Preserve raw whitespace and line endings; validation is not a transformation.\n    fail(typeof p.body === 'string', 'INPUT', '원문은 문자열이어야 합니다.');\n    nonempty(p.body, '원문');\n    const body = p.body,\n      title = nonempty(p.title || body.slice(0, 90), '제목', 180);");
...
if (!install.includes('namespacedMigration')) {
  install = "import { namespacedMigration } from '../../lib/studio/plmag-canonical-db.mjs';\n" + install;
  install = install.replace("  'lib/studio/rep-bridge.mjs',", "  'lib/studio/rep-bridge.mjs',\n  'lib/studio/plmag-canonical-db.mjs',\n  'app/api/studio/asset/route.ts',");
  const anchor = 'const notePath = path.join';
  if (!install.includes(anchor)) throw new Error('Installer anchor drifted.');
  install = install.replace(anchor, `// Existing BU tables may share canonical table names. Never replace them.
...
console.log('Raw text preserved exactly; Plmag migrations use the ocp_ namespace.');
