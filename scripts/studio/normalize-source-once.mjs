...
const edits = {
  'public/ocp-studio/projector.mjs': [
    ['HIERARCHY, TIERS, USER_FLOW, BUILDER_FLOW', 'USER_FLOW, BUILDER_FLOW'],
  ],
  'public/ocp-studio/app.mjs': [
    ['esc(n.url)', 'esc(safeURL(n.url))'],
    [
      '} catch {}',
      '} catch { /* The command already surfaced the server error in the toast. */ }',
    ],
    [
      'api.rep({ mode: "snapshot" })',
      'api.rep({ mode: "snapshot", includeCloud: true })',
    ],
  ],
  'scripts/studio/serve.mjs': [
    ['readFile, writeFile, mkdir', 'readFile, mkdir'],
  ],
...
const manifest = JSON.parse(
  await readFile('docs/STUDIO-V8-SOURCE-MANIFEST.json', 'utf8'),
);
...
await writeFile(
  'docs/STUDIO-V8-SOURCE-MANIFEST.json',
