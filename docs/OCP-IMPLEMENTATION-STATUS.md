...
| Reproducibility contract and separated state planes | Harness baseline locally acceptance-verified | `.ocp/manifest.json`, verification contract, current feature state, capability registry, target registry, and passing `RUN-20260823-001` from committed source | Verify the first GitHub PR, then add dependency/access and recovery registries |
| Runtime Observability Independence | Harness Baseline v1 locally acceptance-verified | `OCP-RC-002`, `OCP-VERIFY-001`, three-viewport direct-route/action evidence, 91 artifact hashes, zero console errors, and zero network failures in `RUN-20260823-001` | Execute the baseline in the first GitHub PR, then connect an external D1/R2-compatible preview provider |
...
| Context Integrity & Runtime Verification Harness | Harness Baseline v1 locally acceptance-verified | `OCP-MOD-INTEGRITY-001` separates six truth planes; `OCP-VERIFY-001` and `RUN-20260823-001` verify the machine entry contract, feature ladder, local runner, and evidence artifact | Passing GitHub run, verified conversation capture, immutable transcript manifest, atomic candidate ledger, and gated state proposal |
