...
test('approved page/module/asset/atom identities survive GUI/CLI assembly; unapproved parts do not', () => {
  const nodes = ['page','module','asset','atom'].map(kind => ({id:kind,kind,decisionId:'reviewed-'+kind,title:kind+' <script>',requirement:{acceptance:'대상 검증'}}));
  const edges = nodes.slice(1).map((n,i) => ({from:nodes[i].id,to:n.id,predicate:'CONTAINS'}));
