...
test('REP raw capture preserves leading spaces, CRLF and trailing newlines byte-for-byte', async () => {
  ...
  const payload = { title: '원문 무결성', body, sourceSystem: 'rep-note', sourceId: 'note-whitespace', sourceVersion: 1 };
  const first = await execute(emptyGraph(), { id: 'exact-first', expectedRevision: 0, type: 'capture', payload }, { actor });
  assert.equal(first.graph.nodes[0].body, body);
  assert.equal(first.graph.nodes[0].source.sha256, await digest(body));
  const second = await execute(first.graph, { id: 'exact-repeat', expectedRevision: first.graph.revision, type: 'capture', payload }, { actor });
